...
const createAccessToken = (
  ...
  return jwt.sign(
    ...
    {
      expiresIn:
        appConfig.access_token_limit as NonNullable<
...
const createRefreshToken = (
  ...
  return jwt.sign(
    ...
    {
      expiresIn:
        appConfig.refresh_token_limit as NonNullable<
          jwt.SignOptions["expiresIn"]
        >,
    }
  );
...
const getRefreshTokenExpirationDate =
  (): Date => {
    const limit =
      appConfig.refresh_token_limit;

    const match = limit.match(
      /^(\d+)([smhdy])$/
    );
    ...
    if (
      ...
      throw new Error(
        "Invalid refresh token expiration unit"
      );
    }
