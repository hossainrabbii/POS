...
import type { ICreateProduct, IUpdateProduct } from "./product.interface.js";
...
export const createProduct = async (payload: ICreateProduct) => {
  ...
  let sku = generateSku(payload.name);

  let skuExists = await Product.findOne({
    sku,
  });
  ...
  while (skuExists) {
    ...
    skuExists = await Product.findOne({
      sku,
    });
  }
