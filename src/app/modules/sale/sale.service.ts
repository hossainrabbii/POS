...
import type { IAddSalePayment, ICreateSale, ISale } from "./sale.interface.js";
...
export const createSale = async (
  ...
  try {
    ...
    for (const item of data.items) {
      ...
      if (!updatedProduct) {
        throw new Error(`Unable to update stock for "${product.name}"`);
      }
    }
    ...
    if (data.dueCommitmentMonths !== undefined && dueAmount === 0) {
      throw new Error(
...
export const getAllSales = async (
  ...
  if (soldBy) {
    if (!Types.ObjectId.isValid(soldBy)) {
